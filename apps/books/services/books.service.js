...
function loadBooks(filteredBy) {
   
    let books = _getBooks()
    
    if (filteredBy) {
        let { bookName, minPrice, maxPrice } = filteredBy
...
function addReview(bookID, review) {
  ...
  getById(bookID)
    .then(book => saveReview(book, review))
...
function saveReview(book, review) {
      ...
      const index = gBooks.indexOf(book.id)

      gBooks[index] = book

      console.log(gBooks[index].reviews);
}
